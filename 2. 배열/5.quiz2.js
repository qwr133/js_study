...
var delateName = prompt(`현재멤버 : ${member} \n 삭제할 이름을 입력하세요`);


    for(var i=0; i<member.length; i++){
        if(`${member}` ===delateName){
            prompt(`삭제완료! \n 남은멤버 : ${member}`)

     }else if(`${member}`!== delateName){
        prompt('잘못된 이름입니다. 다시 입력하세요.');
...
while (tvxq.length > 0) { //if==0 부분 주석처리 해제하고 while(true)라고 입력해도 같음
